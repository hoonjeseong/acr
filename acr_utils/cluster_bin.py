...
def Clustering(Fasta,Coverage,Size,process,jgi):
    ...
    if jgi=='N':
        ...
        assert is_integer(jgi), "please insert the number of samples"
        select=[0]+[i for i in range(3,3+2*(int(jgi)),2)]
        df=pd.read_table(Coverage, delimiter='\t',index_col=0,usecols=select)
    df=df.loc[Bin,:]
...
def Checking_info(Km,SCGs,Out_P,Fa_Size,Marker,Log,ID,f_Score,tag,Path,Ex_contig,sub,Prefix,log_P,N,Ex):
    ...
    for c in Km:
        ...
        Log_F.write('Domain: '+tag+', ID: '+ID.split(Ex)[0]+', number of K:'+str(N)+', Completeness: '+str(Completeness)+', Redundancy: '+str(Redundancy)+', Size: '+str(Size)+', Score: '+str(Score)+', Group: '+ str(sub)+'\n')
        if Score >= f_Score and not bool(set(Km[c]) & set(Ex_contig)):
            NewID=Out_P+'/'+Prefix+'.'+ID.split(Ex)[0]+'-'+str(sub)+'.'+tag+'.fa'
            Write_Fa(Path+'/'+ID,NewID,Km[c])
            Log[str(ID.split(Ex)[0]+'-'+str(sub)+'.'+tag)]={'Domain':tag,'Completeness':Completeness,'Redundancy':Redundancy,'Size':Size,'Score':Score}
            sub+=1
            Ex_contig+=Km[c]
    Log_F.close()
